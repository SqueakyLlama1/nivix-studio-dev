...
function queryItemsUnified({ categoryId = null, rules = [], logicalOp = 'AND' } = {}) {
    ...
    let query = `SELECT i.* FROM items i`;
    const params = [];
    let whereClauses = [];
    ...
    rules.forEach((rule) => {
        ...
        if (rule.type === 'native') {
            ...
            ruleClauses.push(`EXISTS (
                SELECT 1 FROM item_attributes_index idx 
                WHERE idx.item_id = i.id 
                  AND idx.attr_key = ? 
                  AND idx.attr_value ${rule.operator} ?
            )`);
    ...
    const finalQuery = `${query} ${whereSql}`;
