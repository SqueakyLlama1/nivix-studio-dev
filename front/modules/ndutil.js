...
window.ndutil = {
    ...
    writeFile: async (p, content, target = 'default') => {
        warnLegacy(target);
        const cp = Array.isArray(p) ? await window.ndutil.pathjoin(p) : p;
        return sendCommand('write', { path: cp, content, target });
    },
    ...
    deleteFile: async (p, target = 'default') => {
        warnLegacy(target);
        const cp = Array.isArray(p) ? await window.ndutil.pathjoin(p) : p;
        return sendCommand('delete', { path: cp, target });
    },

    createDirectory: async (p, target = 'default') => {
        warnLegacy(target);
        const cp = Array.isArray(p) ? await window.ndutil.pathjoin(p) : p;
        return sendCommand('createDirectory', { path: cp, target });
    },
