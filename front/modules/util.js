    ...
    async checkUpdate() {
        ...
        if (!listExists) {
            try {
                ...
                console.warn(`[CSTDOVER] Failed to cache list.json: ${err}`);
                return;
            }
        }
        ...
        if (util.needsUpdate(studio.sessionVersion, serverVersion)) {
            console.log(`Local Studio Version: ${studio.sessionVersion}`);
            console.log(`Server Studio Version: ${serverVersion}`);
            if (prefs?.loaded["stdoUpdNoti"]) {
                console.warn("Studio update available");
